...
def plotBestFit(wei):
    ...
    y = (-weights[0]-weights[1] * x) / weights[2]# 0=w0x0 + w1x1 + w2x2(X0数据处理时全部补成1，x1相当于x，x2相当于y)
    # y = (5/6)*x+4/0.6
    y=array(y)[0] # 调用gradAscent函数时使用
    ax.plot(x,y)
    plt.xlabel('X1')
    plt.ylabel('X2')
    plt.show()
...
if __name__  == '__main__':
    ...
    weights_res = gradAscent(array(dataArr),labelMat)
